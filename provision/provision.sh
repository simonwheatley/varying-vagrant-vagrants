...
# Configuration for Apache
if [ ! -e /etc/apache/server.key ]; then
	echo "Generate Apache server private key..."
	vvvgenrsa=`openssl genrsa -out /etc/apache2/server.key 2048 2>&1`
	echo $vvvgenrsa
fi
if [ ! -e /etc/apache/server.csr ]; then
	echo "Generate Certificate Signing Request (CSR)..."
	openssl req -new -batch -key /etc/apache2/server.key -out /etc/apache2/server.csr
fi
if [ ! -e /etc/apache/server.crt ]; then
	echo "Sign the certificate using the above private key and CSR..."
	vvvsigncert=`openssl x509 -req -days 365 -in /etc/apache2/server.csr -signkey /etc/apache2/server.key -out /etc/apache2/server.crt 2>&1`
	echo $vvvsigncert
fi

...
for SITE_CONFIG_FILE in $(find /srv/www -maxdepth 4 -name 'vvv-apache.conf'); do
	...
	sed "s#{vvv_path_to_folder}#$DIR#" $SITE_CONFIG_FILE > /etc/apache/custom-sites/$DEST_CONFIG_FILE
done
